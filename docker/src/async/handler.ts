...
import { errorResponse, releaseLeaseFor } from "../proxy/handler.js";
...
/**
 * Pick an account for an off-peak request and return its credentials.
 *
 * The lease is released before returning. Off-peak traffic goes to a SEPARATE
 * origin (`config.async.origin`) through its own client, so it does not consume
 * the account's upstream concurrency — the pool slot was only needed to choose
 * which account to borrow a JWT from. Holding it would keep one slot occupied
 * for the whole poll-and-settle lifetime of every idle-plan task, which is
 * hours; a few of those and the account never dispatches a normal request again.
 */
