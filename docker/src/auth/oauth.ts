  ...
  async complete(_started: OAuthFlowStart, timeoutMs: number = LOGIN_TIMEOUT_MS): Promise<OAuthFlowTokens> {
    ...
    for (;;) {
      ...
      if (data?.status === "ready") {
        ...
        // the provider actually sent (field NAMES only — never the values, so
        // no personal data lands in the log). `pickAccountLabel` reads these.
        if (data.user && typeof data.user === "object") {
          ...
          console.log(`[login] user object fields: ${keys.join(", ") || "(none)"}` +
            (label ? ` — using "${label.replace(/^(.).*(@.*)?$/, "$1***$2")}" as account name` : " — no email/nickname, will use provider-shortId"));
        }
        return {
          accessToken,
