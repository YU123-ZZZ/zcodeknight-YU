...
const TMP = join(tmpdir(), `YU-core-test-${Date.now()}`);
...
`);
    const cfg = loadConfig(path);
    expect(cfg.server.port).toBe(17800);
    // Loopback by default: the panel holds every account credential, and a
    // non-loopback bind is what makes Windows Firewall prompt on each launch.
    // Containers override with ZCODE_PROXY_HOST=0.0.0.0.
    expect(cfg.server.host).toBe("127.0.0.1");
    ...
    expect(cfg.claim).toEqual({
      enabled: true,
      // Auto by default (the operator's call): fresh installs run claim rounds
      // on the poll interval without anyone pressing the panel button. YAML
      // and the panel toggle both override it, and the cooldowns keep a
...
`);
    const cfg = loadConfig(path);
    expect(cfg.identity.appVersion).toBe(DEFAULT_APP_VERSION);
