...
/**
 * PC terminal UI (YU-core's default mode) — an in-process control panel
...
export async function runTui(args: ServeArgs): Promise<void> {
  if (!process.stdout.isTTY || !process.stdin.isTTY) {
    process.stderr.write(
      "YU-core: TUI mode needs an interactive terminal (TTY). " +
        "For headless/CLI use run: YU-core --cli serve\n",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    process.stderr.write(`YU-core: config error: ${(err as Error).message}\n`);
    process.exit(1);
  }
  ...
  if (typeof (stdin as { setRawMode?: (m: boolean) => void }).setRawMode !== "function") {
    realStderrWrite("YU-core: tui requires a terminal with raw-mode input.\n");
    process.exit(1);
  }
  ...
  process.on("uncaughtException", (err: Error) => {
    ...
    cleanup();
    origError(`YU-core: tui crashed: ${err.stack ?? String(err)}`);
    process.exit(1);
  });

  // --- boot ---------------------------------------------------------------------
  console.log(`YU-core TUI — config: ${path}`);
