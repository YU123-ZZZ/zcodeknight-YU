...
import { leasedAccount } from "../auth/multi-auth-manager.js";
...
import { dispatchWithConnectRetry, classifyUpstreamOutcome, reportOutcomeToPool, releaseLeaseFor } from "./handler.js";
...
export async function handleResponses(
  ...
  // ── 4. credential + provider (multi-account aware, mirrors handler.ts) ──
  let cred: Credential;
  let accountId = "";
  try {
    cred = await opts.auth.getCredential();
    ({ id: accountId } = leasedAccount(cred));
  } catch (err) {
    const waitMs = (err as { waitMs?: number }).waitMs;
    const headers = waitMs && waitMs > 0 ? { "retry-after": String(Math.ceil(waitMs / 1000)) } : undefined;
  ...
  const effectivePlan = accountRuntime?.record.plan ?? opts.config.plan;
  const effectiveIdentity: ProxyIdentity = accountRuntime
    ? { ...opts.config.identity, ...(accountRuntime.record.deviceMid ? { deviceMid: accountRuntime.record.deviceMid } : {}) }
    : opts.config.identity;
  const effectiveProviderId = accountRuntime?.record.provider ?? opts.config.provider;
  const providerDef = resolveProviderDef(opts.config, effectiveProviderId);
  ...
  let upstreamRequestBody: string;
  {
    ...
    upstreamRequestBody = transformRequestBody(JSON.stringify(anthropicReq), {
      format: "anthropic",
      metadataUserId: buildAnthropicMetadataUserId(effectiveIdentity.deviceMid, undefined),
      startPlan,
  ...
  const upstreamHeaders = buildUpstreamHeaderPairs(clientReq, upstreamFormat, cred, effectiveIdentity, effectivePlan, captchaHeaders, undefined);
  const upstreamReq = buildUpstreamRequest(clientReq, upstreamFormat, providerDef, cred, transformedBody, effectiveIdentity, effectivePlan, captchaHeaders, undefined);
  ...
  try {
    ...
    releaseLeaseFor(opts.auth, cred);
    ...
    if (await isCaptchaChallenged(upstreamResp, captcha)) {
      ...
      const outcome = await retryOnCaptchaChallenge({
        ...
        solveAndRetry: (retryHeaders) => dispatch(
          buildUpstreamHeaderPairs(clientReq, upstreamFormat, cred, effectiveIdentity, effectivePlan, retryHeaders, undefined),
        ),
        mapError: (err, phase) =>
          phase === "solver"
      ...
      if (!outcome.ok) {
        reportOutcomeToPool(opts.auth, accountId, { kind: "error", message: "captcha solver failed" });
        return outcome.resp;
      }
  ...
  if (!upstreamResp.ok) {
    ...
    reportOutcomeToPool(opts.auth, accountId, classifyUpstreamOutcome(upstreamResp.status));
    releaseLeaseFor(opts.auth, cred);
    return errorResponse(upstreamResp.status, "upstream_error", errText.slice(0, 500) || `upstream returned ${upstreamResp.status}`);
  }
  reportOutcomeToPool(opts.auth, accountId, { kind: "success" });
  opts.auth.touch?.(accountId);
  ...
  if (stream) {
    // The lease is released by the stream itself once it finishes (see
    // StreamResponseContext.onDone) — NOT here. Releasing at this point left a
    // long SSE response holding no slot at all, so the gate under-counted
    // in-flight upstream work for the entire duration of the stream.
    let released = false;
    const onDone = () => {
      if (released) return;
      released = true;
    ...
    const resp = await streamResponse(upstreamResp, { responseId, model: req.model, meta, request: req, input, options: opts, onDone });
    // Exactly-once is guaranteed by the `released` guard: onDone fires from the
    // stream's finally, from its cancel, or (when no stream could be built) from
    // inside streamResponse itself.
    return resp;
  }
...
interface StreamResponseContext {
  ...
  /**
   * Called once the stream is finished, errored, or cancelled.
   *
   * The concurrency lease must be held for as long as the stream runs — the
   * account is still occupied upstream for that whole time — so the release
   * cannot happen where the Response is built. Passing it in keeps this function
   * free of pool knowledge while still releasing on every exit path.
   */
  onDone?: () => void;
}
...
function streamResponse(upstreamResp: Response, context: StreamResponseContext): Response {
  if (!upstreamResp.body) {
    // No stream is built, so nothing will ever call onDone — release here or the
    // lease leaks.
    context.onDone?.();
    return errorResponse(502, "translation_failed", "upstream returned no body for stream");
  }
  ...
  const stream = new ReadableStream<Uint8Array>({
    ...
    cancel(reason) {
      ...
      // A cancelled stream never reaches the finally above (the reader is torn
      // down without the loop finishing), so the lease is released here too.
      // `onDone` is idempotent, so a double call is harmless.
      context.onDone?.();
    },
  });
...
function resolveProviderDef(config: ProxyConfig, providerId?: ProviderId): ProviderDef & { openaiBaseURL: string; anthropicBaseURL: string } {
  const id: ProviderId = providerId ?? config.provider;
  const base = getProvider(id);
  const endpoints = config.providers[id];
  return {
    ...base,
