 ...
 *  2. CDN disk cache under the project data dir (<sha1(url)>) + in-mem cache
...
const CDN_CACHE_DIR = path.join(dataDir(), "captcha-cdn-cache");
...
// Outbound proxying is owned by `network-proxy.ts`, which the engine configures
// at startup from `proxy.*` (falling back to HTTPS_PROXY / HTTP_PROXY). This
// module used to install its own ProxyAgent here; because the global dispatcher
// is last-writer-wins, that could silently undo a proxy configured in the panel.
...
async function createDom(region, prefix) {
  ...
  // Alias pass runs AFTER polyfills so polyfilled props (Option, Video,
  // alert, ...) are visible to guest scripts via globalThis too.
  //
  // Required on EVERY runtime, not just Bun. GlobalWindow has no VM realm
  // (setupVMContext is a no-op) and its evaluateScript is a plain host-realm
  // eval — under Bun that was covered by this aliasing, but the old
  // `typeof Bun !== "undefined"` gate skipped it under Node on the mistaken
  // belief that Node's VM context resolves bare identifiers natively. It does
  // not: the Aliyun SDK died with "window is not defined", every mint timed
  // out, and claims, probes and playground requests all 503'd on the Node
  // bundle. installGlobalWindowAlias's skip list already protects the host
  // internals Node needs (process, timers, Buffer, ...).
...
function destroyDom(win) {
  ...
  try {
    // Unconditional: the alias install is unconditional too, and a Node run
    // that skipped this would leave stale window getters on globalThis.
