...
export function buildServerOptions(
  ...
  auth: AuthManager,
  debug: boolean,
  /** YAML path this process was started with, so panel edits can persist. */
  configPath?: string,
): { config: ProxyConfig; auth: AuthManager; debug: boolean; responseStore?: ResponseStore; configPath?: string } {
  const opts: { config: ProxyConfig; auth: AuthManager; debug: boolean; responseStore?: ResponseStore; configPath?: string } = { config, auth, debug };
